...
**Spoken replies (TTS)**: toggle with `/voice on` / `/voice off`. When on, a voice message you send gets a spoken reply *in addition to* the usual text answer — a typed message always gets a typed-only answer, even with voice mode on, so the conversation follows how you're talking rather than switching everything to voice at once. Three backends via `TTS_PROVIDER`:
